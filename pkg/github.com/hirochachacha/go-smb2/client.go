...
func (d *Dialer) Dial(tcpConn net.Conn) (*Session, string, error) {
...
// This implementation doesn't support multi-session on the same TCP connection.
// If you want to use another session, you need to prepare another TCP connection at first.
func (d *Dialer) DialContext(ctx context.Context, tcpConn net.Conn) (*Session, string, error) {
	if ctx == nil {
		panic("nil context")
	}
	if d.Initiator == nil {
	...
	s, version, err := sessionSetup(conn, d.Initiator, ctx)
	if err != nil {
		return nil, version, err
	}

	return &Session{s: s, ctx: context.Background(), addr: tcpConn.RemoteAddr().String()}, version, nil
}
