...
func sessionSetup(conn *conn, i Initiator, ctx context.Context) (*session, string, error) {
	...
	curversion = getWinVersion(res)

	r := SessionSetupResponseDecoder(res)
	if r.IsInvalid() {
		return nil, curversion, &InvalidResponseError{"broken session setup response format"}
	}

	sessionFlags := r.SessionFlags()
	if conn.requireSigning {
		if sessionFlags&SMB2_SESSION_FLAG_IS_GUEST != 0 {
			return nil, curversion, &InvalidResponseError{"guest account doesn't support signing"}
		}
		if sessionFlags&SMB2_SESSION_FLAG_IS_NULL != 0 {
			return nil, curversion, &InvalidResponseError{"anonymous account doesn't support signing"}
		}
	}
	...
	if err != nil {
		return nil, curversion, &InvalidResponseError{err.Error()}
	}
	...
	if s.sessionFlags&(SMB2_SESSION_FLAG_IS_GUEST|SMB2_SESSION_FLAG_IS_NULL) == 0 {
		...
		case SMB300, SMB302:
			...
			if err != nil {
				return nil, curversion, &InternalError{err.Error()}
			}
...
func getWinVersion(res []byte) string {

	if len(res) < 96 {
		return ""
	}

	VersionBytes := res[87:95]
	...
	NowVersion := fmt.Sprintf("Windows %v.%v Build %v", ServerOSMajor, ServerOSMinor, ServerOSBuild)
