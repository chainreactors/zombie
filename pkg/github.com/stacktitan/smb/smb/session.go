...
import (
	"Zombie/pkg/github.com/stacktitan/smb/gss"
	"Zombie/pkg/github.com/stacktitan/smb/ntlmssp"
	"Zombie/pkg/github.com/stacktitan/smb/smb/encoder"
	"Zombie/src/Utils"
	"bufio"
	"bytes"
	"encoding/asn1"
...
func NewSession(opt Options, debug bool) (s *Session, err error) {
	...
	conn, err := net.DialTimeout("tcp", fmt.Sprintf("%s:%d", opt.Host, opt.Port), time.Duration(Utils.Timeout)*time.Second)
	if err != nil {
		return
	}
...
func (s *Session) NegotiateProtocol() error {
	...
	if !hasNTLMSSP {
		return errors.New("Database does not support NTLMSSP")
	}
