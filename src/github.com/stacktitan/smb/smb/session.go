...
import (
	...
	"Zombie/src/github.com/stacktitan/smb/gss"
	"Zombie/src/github.com/stacktitan/smb/ntlmssp"
	"Zombie/src/github.com/stacktitan/smb/smb/encoder"
)
...
func NewSession(opt Options, debug bool) (s *Session, err error) {
	...
	conn, err := net.DialTimeout("tcp", fmt.Sprintf("%s:%d", opt.Host, opt.Port), 2*time.Second)
	if err != nil {
		return
	}
